...
- [x] **Step 1: Write failing Rust tests** that inject a removal failure after the first target and assert every original file and lock are restored, then assert explicit rollback restores a completed uninstall.
- [x] **Step 2: Run** `cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml uninstall` **and verify failure because uninstall currently removes files directly.**
- [x] **Step 3: Implement** staging, backup, journal transitions, atomic removal, lock retention until commit, rollback, and `needs_manual_recovery` with backup location.
- [x] **Step 4: Run the focused Rust tests and confirm pass.**
- [x] **Step 5: Commit** the transactional uninstall implementation.
...
- [x] **Step 1: Write failing tests** for creating Skill/Prompt/context files, validating paths and manifest declarations, editing content, saving a second immutable revision, and resubmitting a changes-requested publication.
- [x] **Step 2: Run the focused capability-kit and desktop tests and confirm failure because the authoring API/page does not exist.**
- [x] **Step 3: Implement** a three-component Markdown editor with manifest metadata, live validation, local save, server revision save, scan report, version diff, and publication actions.
- [x] **Step 4: Run focused tests and desktop build.**
- [x] **Step 5: Commit** the authoring workspace.
...
- [x] **Step 1: Add failing shared-policy and desktop tests** covering each scan category and asserting no cloud method is called when a blocking finding exists.
- [x] **Step 2: Run focused tests and verify the missing categories fail.**
- [x] **Step 3: Implement** one policy result model shared by TypeScript clients and mirrored by Rust discovery, including evidence digests without secret values.
- [x] **Step 4: Run package, desktop, and security gate tests.**
- [x] **Step 5: Commit** complete pre-upload scanning.
...
- [x] **Step 1: Write failing tests** for network failure persistence, restart recovery, exponential retry, exactly-once completion, permanent authorization failure, and status UI.
- [x] **Step 2: Run focused tests and verify there is no runtime producer/consumer.**
- [x] **Step 3: Implement** durable enqueue before network dispatch, online/startup draining, capped backoff, dead-letter state, manual retry, and cache permission revalidation.
- [x] **Step 4: Run focused desktop/Rust tests and build.**
- [x] **Step 5: Commit** offline synchronization.
...
- [x] **Step 1: Write failing tests** proving production rejects absent SMS configuration, E.164 validation, provider request shape, idempotency header, timeout, retryable status classification, and redacted logs.
- [x] **Step 2: Run API tests and confirm failure.**
- [x] **Step 3: Implement** provider injection for verification, invitations, and worker notifications; add production Compose secrets/configuration.
- [x] **Step 4: Run API tests, typecheck, and Compose config validation.**
- [x] **Step 5: Commit** real SMS delivery.
...
- [x] **Step 1: Write failing tests** for verification/invite/recovery throttles, stale authentication rejection for owner/role operations, replay, concurrent reservation, mismatched payload conflict, and expiry.
- [x] **Step 2: Run focused API tests and verify failures.**
- [x] **Step 3: Implement** reusable guards/interceptor and apply them to every mutating route; retain domain-level publication/install idempotency as defense in depth.
- [x] **Step 4: Run API tests and tenant/security suites.**
- [x] **Step 5: Commit** security controls and idempotency.
...
- [x] **Step 3: Implement** Nest Swagger generation, deterministic SDK generation, shared auth/tenant/idempotency transport, and migrate clients without changing UI-facing interfaces.
- [x] **Step 4: Run SDK tests, client tests, typecheck, and build.**
- [x] **Step 5: Commit** OpenAPI and shared SDK.
...
- [x] **Step 1: Write failing tests** for every job type, duplicate delivery, capped retry, dead-letter visibility, update notification fan-out, and aggregate replacement.
- [x] **Step 2: Run focused tests and verify unsupported jobs fail.**
- [x] **Step 3: Implement** persisted job state, processors, metrics, administrator operations endpoints/UI, and retention-safe archival.
- [x] **Step 4: Run API/Web tests and worker integration checks.**
- [x] **Step 5: Commit** the completed worker platform.
...
- [x] **Step 1: Write failing tests** for owner-only export/transfer/closure, recent-auth enforcement, immutable audit events, cancellation, grace expiry deletion, storage cleanup, and UI confirmation.
- [x] **Step 2: Run focused API/Web tests and verify missing flows fail.**
- [x] **Step 3: Implement** lifecycle state and worker deletion with referential cleanup; expose guarded Web settings controls.
- [x] **Step 4: Run API/Web/security/tenant tests.**
- [x] **Step 5: Commit** data lifecycle management.
...
- [x] **Step 1: Write failing acceptance tests** that call the real Rust command surface, prove clean update, modified-file conflict with diff/import/recovery, transactional uninstall, updater error states, and artifact metadata validation.
- [x] **Step 2: Run the focused acceptance and release tests and verify the old mocked/string-only checks fail.**
- [x] **Step 3: Implement** updater commands/UI, release matrices and signing gates, real runtime harness, and accurate acceptance reporting.
- [x] **Step 4: Run** `pnpm release:verify`, desktop Rust tests, desktop Playwright, Compose health/smoke, CLI packaging, and artifact checksum verification.
- [x] **Step 5: Commit** release readiness and final acceptance evidence.
