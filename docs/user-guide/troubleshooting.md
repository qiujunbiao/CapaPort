...
确认 API `/health/ready` 返回 200；开发环境确认 Mailpit 收到最新验证码。验证码过期后重新发送，不要重复使用。CLI 运行 `agentdoor doctor` 查看脱敏诊断。
...
说明文件自上次安装后被修改。查看差异后选择保留、导入个人草稿或恢复。Agentdoor 不会静默覆盖本地编辑。
