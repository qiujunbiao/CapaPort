...
export function SecurityPage({
  ...
  return (
    <div className="page">
      ...
      {saved ? <div role="status"><Status tone="good">安全策略已保存</Status></div> : null}
      <div className="security-summary">
        <Panel>
          <ShieldCheck />
          <span><strong>客户端与云端双重扫描</strong><small>组织规则无法由客户端绕过</small></span>
          <Status tone="good">启用</Status>
        </Panel>
        <Panel>
          <KeyRound />
          <span><strong>旋转刷新令牌</strong><small>重放会撤销令牌链</small></span>
          <Status tone="good">启用</Status>
        </Panel>
        <Panel>
          <AlertTriangle />
          <span><strong>失败任务</strong><small>可重试通知与事件投递</small></span>
          <Status tone={deadLetters?.length ? 'warn' : 'good'}>{deadLetters?.length ?? 0}</Status>
        </Panel>
      </div>
      <Panel>
        <div className="panel-title"><div><p className="eyebrow">ORGANIZATION POLICY</p><h2>组织扫描策略</h2></div></div>
        {!policy ? <LoadingBlock label="读取扫描策略" /> : (
          <div className="form-grid">
            <label>可执行文件策略
              <select
                value={policy.executablePolicy}
                disabled={!canManage}
                onChange={(event) => setPolicy({ ...policy, executablePolicy: event.target.value as OrganizationSecurityPolicy['executablePolicy'] })}
              >
            ...
            <label>组织禁止词
              <textarea rows={4} value={blockedTerms} disabled={!canManage} onChange={(event) => setBlockedTerms(event.target.value)} />
            </label>
            <label>允许访问的网络主机
              <textarea rows={4} value={allowedHosts} disabled={!canManage} onChange={(event) => setAllowedHosts(event.target.value)} />
            </label>
            <label>允许的可执行文件路径
              <textarea rows={4} value={allowedPaths} disabled={!canManage} onChange={(event) => setAllowedPaths(event.target.value)} />
            </label>
            {canManage ? <Button onClick={() => void savePolicy()}>保存安全策略</Button> : <p className="quiet-copy">审计员拥有只读权限。</p>}
          </div>
        )}
      </Panel>
      <Panel>
        <div className="panel-title"><div><p className="eyebrow">PUBLICATION RISK</p><h2>发布风险报告</h2></div></div>
        {!risks ? <LoadingBlock label="读取发布风险" /> : risks.length ? (
          <div className="session-list">
            {risks.flatMap(({ publication, findings }) => findings.map((finding, index) => (
              <article key={`${publication.id}:${finding.ruleId}:${finding.path}:${index}`}>
                <span><strong>{finding.ruleId}</strong><small>{publication.version} · {finding.path} · {finding.message}</small></span>
      ...
      <Panel>
        <div className="panel-title"><div><p className="eyebrow">ACCOUNT SESSIONS</p><h2>登录会话</h2></div></div>
        {!sessions ? <LoadingBlock label="读取会话" /> : sessions.length ? (
          <div className="session-list">
            {sessions.map((session) => (
              <article key={session.id}>
                <span className="section-icon"><Laptop /></span>
                <div><strong>{session.deviceName || '浏览器会话'}</strong><small>最近活动 {new Date(session.lastSeenAt).toLocaleString()}</small></div>
                {session.current ? <Status tone="good">当前会话</Status> : (
                  <Button variant="danger" onClick={async () => { await client.revokeSession(session.id); await load(); }}>撤销</Button>
                )}
              </article>
            ))}
          </div>
      ...
      <Panel>
        <div className="panel-title"><div><p className="eyebrow">DEAD LETTER VISIBILITY</p><h2>失败任务</h2></div></div>
        {deadLetters?.length ? (
          <div className="session-list">
            {deadLetters.map((job) => {
              const kind = String(job.kind ?? 'operation') as 'operation' | 'outbox' | 'delivery';
              const id = String(job.id ?? '');
              return <article key={`${kind}:${id}`}>
                <span><strong>{String(job.type ?? job.event_type ?? job.channel ?? kind)}</strong><small>{kind} · 尝试 {String(job.attempts ?? 0)} 次 · {String(job.last_error ?? job.error_code ?? '失败')}</small></span>
                <Button onClick={async () => { await client.retryDeadLetter(kind, id); await load(); }}>重新执行</Button>
              </article>;
            })}
          </div>
