...
WORKDIR /workspace
COPY . .
RUN --mount=type=cache,id=agentdoor-web-pnpm,target=/root/.local/share/pnpm/store \
...
LABEL org.opencontainers.image.title="Agentdoor Web" \
      org.opencontainers.image.description="Agentdoor organization administration console"
